...
function normalizeVocabularyUnits(data = []) {
...
function normalizeVocabularyWords(data = []) {
...
function createStudySession(words = [], now = Date.now()) {
  return {
    flow: [StudyStep.learn, StudyStep.choose, StudyStep.spell],
...
function buildChooseOptions(word, allWords = []) {
...
function evaluateChoice(word, selectedMeaning) {
  return String(selectedMeaning || '').trim() === String(word.meaning || '').trim();
}

function getVisibleSpellingHint(answer, hintLevel = 0) {
...
function evaluateSpelling(word, input, wrongCount = 0) {
...
function getStudyResult(session = {}) {
  const results = arrayFrom(session.wordResults);
  const total = results.length;
  const passed = results.reduce((sum, item) => sum + (item.choosePassed ? 1 : 0) + (item.spellPassed ? 1 : 0), 0);
  const possible = total * 2;
  return {
    total,
    correctRate: possible ? Math.round((passed / possible) * 100) : 0,
    durationMinutes: Math.max(1, Math.round((Number(session.finishedAt || Date.now()) - Number(session.startedAt || Date.now())) / 60000)),
  };
}

...
export function normalizeDashboard(payload = {}, now = Math.floor(Date.now() / 1000)) {
  ...
  return {
    ...
    users: users.map((user) => ({
      ...
      avatar: user.avatar || '',
      mobile: user.mobile || '',
      currentBook: user.currentBook || user.book_name || '-',
      currentPage: toNumber(user.currentPage ?? user.cur_page),
      createTime: toNumber(user.create_time || user.createTime),
      lastLoginTime: toNumber(user.lastLoginTime ?? user.last_login_time),
      memberExpireTime: toNumber(user.member_expire_time || user.memberExpireTime),
      memberState: user.memberState || user.member_state || getMemberState(user.member_expire_time || user.memberExpireTime, now),
      inviteCount: toNumber(user.inviteCount ?? user.invite_count),
      adWatchCount: toNumber(user.adWatchCount ?? user.ad_watch_count),
      rewardMinutes: toNumber(user.rewardMinutes ?? user.reward_minutes),
      remainingMinutes: toNumber(user.remainingMinutes ?? user.remaining_minutes),
      memberExchangeCount: toNumber(user.memberExchangeCount ?? user.member_exchange_count),
    })),
    plans,
    orders,
